...
namespace MeerPflege.API.Controllers
{
  ...
  public class NursesController : BaseApiController
  {
    ...
    public async Task<ActionResult<NurseDto>> GetProfileInfo()
    {
      var email = GetEmail(HttpContext);
      var user = await _userManager.FindByEmailAsync(email);

      var result = await Mediator.Send(new List.Query() { Id = user.NurseId });
      var data = Application.Core.Result<NurseDto>.Success(result.Value.FirstOrDefault());
